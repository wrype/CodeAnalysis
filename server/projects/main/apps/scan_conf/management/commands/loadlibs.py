...
SCAN_CONF_COMMANDS_PATH = os.path.join(
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "lib_name_list", type=str, nargs="+", help="需要load的工具依赖列表"
        )
        parser.add_argument(
            "-ignore-auth", "--ignore-auth", action="store_true", help="忽略凭证"
        )
    ...
    def get_or_create_account(self, account, password, user):
        ...
        scm_account = ScmAccount.objects.filter(
            user=user, scm_username=account, auth_origin_id=settings.DEFAULT_ORIGIN_ID
        ).first()
        created = False
        if not scm_account:
    ...
    def handle(self, *args, **options):
        ...
        for lib_name in lib_name_list:
            try:
                ...
                if ignore_auth is not True:
                    # 存在非link类型的依赖，则需要凭证
                    if (
                        toollib_json.get("scm_type") != ToolLib.ScmTypeEnum.LINK
                        and not scm_account
                    ):
                        if account and password:
                            scm_account, _ = self.get_or_create_account(
                                account, password, codedog
                            )
                        else:
        ...
        load_res_list = ToolLibLoadManager.loadlib_by_workers(
            toollib_json_list, user=codedog
        )
